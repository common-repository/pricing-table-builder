...
define('kCActiveRecordStateNew', 1);
define('kCActiveRecordStateClean', 2);
define('kCActiveRecordStateDirty', 3);
define('kCActiveRecordStateRemoved', 4);

abstract class SGPT_ActiveRecord
{
	private $ar_state = 0;
	...
	{
		$this->ar_state = kCActiveRecordStateNew;
	}
	...
	public function __call($name, $args)
	{
		...
		if ($methodPrefix=='get') {
			...
			if ($this->ar_state==kCActiveRecordStateRemoved) return;

			if(count($args)==1 && $this->$methodProperty!=$args[0]) {
				if ($this->ar_state==kCActiveRecordStateClean) $this->ar_state = kCActiveRecordStateDirty;
	...
	protected function getTableName()
	{
		...
		if ($class->hasConstant(self::TABLE_CONST)) {
			return $wpdb->prefix.$sgpt->prefix.$class->getConstant(self::TABLE_CONST);
		}
	...
	/**
     *
     * Create an SGPT_ActiveRecordCriteria object from a string and params
     *
     * @param string $criteria  the condition of the criteria
     * @param array $params  parameters
     * @return SGPT_ActiveRecordCriteria object
     */
	private function prepareCriteria($criteria='', $params=array())
	{
		if (!$criteria instanceof SGPT_ActiveRecordCriteria) {
			$criteria = new SGPT_ActiveRecordCriteria($criteria, $params);
		}
	...
	private function instantiate($data)
	{
		...
		$obj->ar_state = kCActiveRecordStateClean;
	 ...
	 *
     * @param string/SGPT_ActiveRecordCriteria $criteria  SQL condition or criteria object
	...
	{
		if ($this->ar_state==kCActiveRecordStateNew) { //insert
			$values = array();
			$params = array();
		...
		else if ($this->ar_state==kCActiveRecordStateDirty) { //update
...
/**
 * SGPT_ActiveRecordCriteria
 */
class SGPT_ActiveRecordCriteria
{
